...
class ServerSideTable(object):
    ...
    def _run(self, data):
        ...
        for i in range(len(self.columns)):
            if len(self.request_values.get(f'sSearch_{i}', "")) != 0:
                filtered_data = self._custom_filter(data, i)
                data = filtered_data
        if len(filtered_data) == 0:
            filtered_data = self._custom_filter_all(data)
    ...
    def _custom_filter(self, data, i):
        '''
        ...
        def check_row(row, i):
            ...
            if self.columns[i]['searchable']:
                ...
                regex = self.request_values[f'sSearch_{i}']
                if str(regex).lower() in str(value).lower():
                    return True
            return False

        if self.request_values.get(f'sSearch_{i}', ""):
            return [row for row in data if check_row(row, i)]
        else:
            return []

    def _custom_filter_all(self, data):
        '''
        Filters out those rows that do not contain the values specified by the
        user using a case-insensitive regular expression.
        It takes into account only those columns that are 'searchable'.
