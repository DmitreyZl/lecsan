...
from app.app.models import Order1, Delivery6, Driver, OrdersOzon, OrdersYM, ImportProvider1, Rule3, Items, \
    ItemsProvider1, Vendor1, Module, Profile1, Transport
from app.app.helper import ServerSideTable
from app.app.models_table import SERVERSIDE_TABLE_COLUMNS, SERVERSIDE_TABLE_VENDOR_COLUMNS, \
        ...
        if form.is_valid():
            ...
            if user:
                ...
                if request.GET.get('next', 0) != 0:
                    return redirect(request.GET.get('next', 0))
                else:
                    return redirect('/')
            else:
                return render(request, 'login.html', {'form': form})
...
def ozon_order(request):
    ...
    query = list(OrdersOzon.objects.filter(number_id=request.GET.get('id', 0)).values())
...
def ym_order(request):
    query = list(OrdersYM.objects.filter(number_id=request.GET.get('id', 0)).values())
    r = requests.get(
...
def ym_orders(request):
    query = list(OrdersYM.objects.values())
...
def ozon_orders(request):
    client_id = request.GET.get('client_id', '258')
    query = list(OrdersOzon.objects.filter(client_id=client_id).values())
...
def statistics_items(request):
    data = list(ImportProvider1.objects.filter(site=request.GET.get('site', 'технолайн')).values('name', 'date_register',
                                                                                                 'count_new', 'count_update'))
...
def my_items(request):
    ...
    vendors = ItemsProvider1.objects.exclude(id_site='').filter(
        story=request.GET.get('site', 'boint')).values('id_vendor', 'vendor_name',
                                                       'name', 'price', 'oc_price',
                                                       'price_site', 'quantity',
                                                       'id_site')
    build_list = []
    ...
    for v in vendors:
        if v['id_site'] not in build_list:
            dict_build = data.filter(id_site=v['id_site']).values('id_site', 'brand', 'section', 'name',
                                                                  'article', 'price', 'quantity', 'status')
            if len(dict_build) == 0:
                continue
    ...
    data_list = data_list + list(data.exclude(id_site__in=build_list).values('id_site', 'brand', 'section', 'name',
                                                                             'article', 'price', 'quantity', 'status'))
    vendors_list = Vendor1.objects.filter(active='Активен').values('name')
...
def my_vendor_items(request):
    ...
    data, data_items = [], []
    data_items = list(Items.objects.filter(story=request.GET.get('site', 'boint')).values('id_site', 'brand', 'section',
                                                                                          'name', 'article', 'price',
                                                                                          'quantity', 'story'))
    if request.GET.get('name_item', 0) == 0:
        name_item = '-'
        ...
        name_item = request.GET.get('name_item', 0).replace('_', ' ')
    data = list(ItemsProvider1.objects.filter(
        story=request.GET.get('site', 'boint')).filter(id_site='').values('id_vendor', 'vendor_name', 'name',
                                                                          'article', 'price', 'oc_price', 'price_site',
                                                                          'quantity', 'date_update'))
    vendors = list(Vendor1.objects.filter(active='Активен').values('name'))
...
def wildberries_items(request):
    data = Items.objects.filter(story='wildberries').values('id_site', 'brand', 'section', 'name', 'article',
                                                            'price', 'quantity', 'status', 'willdberis', 'is_kgt')
    global load_boint_items
    load_boint_items = Items.objects.filter(story='boint').values('id_site', 'brand', 'section', 'name', 'price',
                                                                  'quantity')
    if request.GET.get('name_item', 0) == 0:
        name_item = '-'
    else:
...
def my_vendors(request):
    data = list(Vendor1.objects.values())
    transport = list(Transport.objects.values())
...
def my_profiles(request):
    ...
    data = list(Profile1.objects.filter(site=request.GET.get('site', 'технолайн')).values())
    ...
    context = {'df': data, 'module': module, 'story_category': category_site,
               ...
               'story_brand': characters_ozon, 'brand': list(characters_site['Название'].unique()),
               'site': site, 'market': 'Ozon'}
    rendered_page = template.render(context, request)
    return HttpResponse(rendered_page)
...
def ym_profiles(request):
    ...
    site = ['технолайн', 'boint', 'mainfled.spb']
    data = list(Profile1.objects.filter(module='YandexMarket').values())
    template = loader.get_template('export_profile.html')
    context = {'df': data,
               'story_brand': characters_ym, 'brand': list(characters_site['Название'].unique()),
               'brand_tech': list(characters_site_tech['Название'].unique()),
               'site': site, 'market': 'YandexMarket'}
    rendered_page = template.render(context, request)
    return HttpResponse(rendered_page)
...
def my_vendor(request):
        df = pd.read_csv(config['EXPORT_SITE'][request.GET.get('site', 'технолайн')] +
                         'bitrix/catalog_export/lecsan_export/catalogs/export_new.csv')
        story_vendor = list(df['Бренд'].unique())
        section = list(df['Категория'].unique())
        transports = list(Transport.objects.values('name'))
        vendor = list(Vendor1.objects.filter(name=request.GET.get('vendor', 0)).values('name', 'cron', 'transport',
                                                                                        'file_name'))
        transport = list(Transport.objects.filter(name=vendor[0]['transport']).values('name', 'host', 'port', 'login',
                                                                                      'password', 'path', 'name_file'))
        rules = list(Rule3.objects.filter(vendor=request.GET.get('vendor', 0)).values('id_rule', 'story',
                                                                                       'vendor_site', 'section', 'type',
                                                                                       'margin', 'type_margin'))
        template = loader.get_template('vendor.html')
        context = {'transport': transport[0], 'transports': transports, 'vendors': vendor, 'rule': rules,
                   'site': request.GET.get('site', 'технолайн'), 'section': section, 'story_vendor': story_vendor}
        rendered_page = template.render(context, request)
        return HttpResponse(rendered_page)
...
def load(request):
    global data_list
    ret = ServerSideTable(request.GET, data_list, SERVERSIDE_TABLE_COLUMNS).output_result()
    return JsonResponse(ret)


def load_boint_items(request):
    global load_boint_items
    ret = ServerSideTable(request.GET, load_boint_items, SERVERSIDE_TABLE_BOINT_ITEMS_COLUMNS).output_result()
    return JsonResponse(ret)


def load_items(request):
    global data_items
    ret = ServerSideTable(request.GET, data_items, SERVERSIDE_TABLE_ITEMS_COLUMNS).output_result()
    return JsonResponse(ret)


def load_vendor_items(request):
    global data
    ret = ServerSideTable(request.GET, data, SERVERSIDE_TABLE_VENDOR_COLUMNS).output_result()
    return JsonResponse(ret)
